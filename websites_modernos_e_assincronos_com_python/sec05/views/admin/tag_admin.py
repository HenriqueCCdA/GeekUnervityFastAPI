...
class TagAdmin(BaseCrudView):
    ...
    def __init__(self) -> None:
        ...
        self.router.routes.append(Route(path='/tag/details/{obj_id:int}', endpoint=self.object_edit, methods=["GET",], name='tag_details'))
        self.router.routes.append(Route(path='/tag/edit/{obj_id:int}', endpoint=self.object_edit, methods=["GET", "POST"], name='tag_edit'))
        self.router.routes.append(Route(path='/tag/delete/{obj_id:int}', endpoint=self.object_delete, methods=["DELETE",], name='tag_delete'))
    ...
    async def object_delete(self, request: Request) -> Response:
        ...
        return await super().object_delete(object_controller=tag_controller, obj_id=obj_id)
    ...
    async def object_edit(self, request: Request) -> Response:
        ...
        # Se o request for GET
        if request.method == 'GET':
            return await super().object_details(object_controller=tag_controller, obj_id=obj_id)

        # Se o request for POST
        tag_obj = await tag_controller.get_one_crud(id_obj=obj_id)
